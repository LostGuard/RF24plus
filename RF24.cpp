...
#ifdef SERIAL_DEBUG
  char str[80];
  sprintf(str, "write_register(%02x,%02x)\r\n", reg, value);
  _debug->print(str);
#endif
...
uint8_t RF24::write_payload(const void* buf, uint8_t len)
{
  ...
  {
    char buf[80];
    sprintf(buf, "[Writing %u bytes %u blanks]",data_len,blank_len);
    _debug->print(buf);
  }
  _gpio->csn(LOW);
...
uint8_t RF24::read_payload(void* buf, uint8_t len)
{
  ...
  {
    char buf[80];
    sprintf(buf, "[Reading %u bytes %u blanks]",data_len,blank_len);
    _debug->print(buf);
  }
  _gpio->csn(LOW);
...
void RF24::print_status(uint8_t status)
{
  ...
  {
    char buf[80];
    sprintf(buf, "STATUS\t\t = 0x%02x RX_DR=%x TX_DS=%x MAX_RT=%x RX_P_NO=%x TX_FULL=%x\r\n",
               status,
...
void RF24::print_observe_tx(uint8_t value)
{
  ...
  {
    char buf[80];
    sprintf(buf, "OBSERVE_TX=%02x: POLS_CNT=%x ARC_CNT=%x\r\n",
               value,
...
void RF24::print_byte_register(const char* name, uint8_t reg, uint8_t qty)
{
  if (_debug)
  {
    ...
    sprintf(buf, "%s\t%c =", name, extra_tab);
    _debug->print(buf);
    while (qty--)
    {
      sprintf(buf, " 0x%02x", read_register(reg++));
      _debug->print(buf);
    }
    _debug->print("\r\n");
  }
...
RF24::RF24(RF24_GPIO* gpio, RF24_SPI* spi, RF24_TIMER* timer, RF24_DEBUG* debug):
...
void RF24::printDetails(void)
{
  ...
  char buf[80];
  sprintf(buf, "Data Rate\t = %s\r\n", pgm_read_word(&rf24_datarate_e_str_P[getDataRate()]));
  _debug->print(buf);
  sprintf(buf, "Model\t\t = %s\r\n", pgm_read_word(&rf24_model_e_str_P[isPVariant()]));
  _debug->print(buf);
  sprintf(buf, "CRC Length\t = %s\r\n", pgm_read_word(&rf24_crclength_e_str_P[getCRCLength()]));
  _debug->print(buf);
  sprintf(buf, "PA Power\t = %s\r\n", pgm_read_word(&rf24_pa_dbm_e_str_P[getPALevel()]));
  _debug->print(buf);
}
...
void RF24::begin(void)
{
  if (_gpio->err != 0)
  {
    _debug->print("an error occured during initialization! Aborting! \r\n");
     return;
  }
  // just to simulate arduino milis()
  //RF24_TIME::__start_timer();
  // Initialize pins
...
bool RF24::write(const void* buf, uint8_t len)
{
  ...
  while (!(status & (_BV(TX_DS) | _BV(MAX_RT))) && (_timer->CheckForTimeOut() == false));
...
void RF24::startWrite(const void* buf, uint8_t len)
{
  ...
  write_register(CONFIG, cfg);
  _timer->usleep(2);
  write_register(CONFIG, cfg & ~_BV(PRIM_RX));
  _timer->usleep(150);
...
bool RF24::testRegister(uint8_t reg, uint8_t val)
{
